...
from services.mcp_client import MCPClient # Assuming mcp_client.py is in services/

# Configuration class for MCP server parameters
class MCPServerParams:
    def __init__(self, name: str, transport: str, command: Optional[List[str]] = None, url: Optional[str] = None, headers: Optional[Dict[str, str]] = None):
        self.name = name
...
class BaseMCPAgent:
    def __init__(self, name: str, mcp_client: MCPClient, server_params: MCPServerParams, llm_config: Dict[str, Any], system_message: str):
        self.name = name
        ...
        self.executor_agent = ConversableAgent(
            ...
            is_termination_msg=lambda x: x.get("content", "").rstrip().endswith("TERMINATE"),
            human_input_mode="NEVER",
            code_execution_config={"use_docker": False} # Assuming no docker for now
        )
        ...
        self.logger.info(f"BaseMCPAgent {name} initialized with server {server_params.name}")

    # Synchronous wrappers for async MCPClient methods
    def list_tools_sync(self) -> List[Dict[str, Any]]:
        """List available tools from the MCP server."""
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(self.mcp_client.list_tools(self.server_params.name))

    def call_tool_sync(self, tool_name: str, **kwargs) -> Any:
        """Call a tool on the MCP server."""
        loop = asyncio.get_event_loop()
        # The kwargs will contain the parameters for the tool call
        return loop.run_until_complete(self.mcp_client.call_tool(self.server_params.name, tool_name, kwargs))

    def read_resource_sync(self, uri: str) -> str:
        """Read a resource from the MCP server."""
        loop = asyncio.get_event_loop()
        # The read_resource method in MCPClient returns a dict, extract content
        result = loop.run_until_complete(self.mcp_client.read_resource(self.server_params.name, uri))
...
class AssistantMCPAgent(BaseMCPAgent):
    def __init__(self, name: str, mcp_client: MCPClient, server_params: MCPServerParams, llm_config: Dict[str, Any], system_message: str):
        ...
        self.logger.info(f"AssistantMCPAgent {name} initialized with enhanced system message")


    def _enhance_system_message(self):
        """Fetches tools and enhances the system message with their descriptions."""
        self.logger.info(f"Fetching tools for {self.server_params.name} to enhance system message")
        try:
            available_tools = self.list_tools_sync() # Use the sync wrapper
            if not available_tools:
                self.logger.warning(f"No tools found for server {self.server_params.name}. System message not enhanced.")
                return
            ...
            for tool in available_tools:
                ...
                for param_name, param_info in parameters.items():
                    ...
                    if param_description:
                        param_infos.append(f"- {param_name} ({param_type}): {param_description}")
                    else:
            ...
            self.logger.info(f"System message enhanced with tools from {self.server_params.name}")

        except Exception as e:
            self.logger.error(f"Error enhancing system message with tools: {e}")
            # Continue with the original system message if tool fetching fails

    def initiate_chat(self, recipient: ConversableAgent, message: str, **kwargs):
        """Initiates a chat using the caller agent."""
        self.logger.info(f"Initiating chat from {self.name} with message: {message}")
        return self.caller_agent.initiate_chat(recipient, message=message, **kwargs)

    def send(self, message: str, recipient: ConversableAgent, **kwargs):
         """Sends a message using the caller agent."""
         self.logger.info(f"Agent {self.name} sending message to {recipient.name}: {message}")
