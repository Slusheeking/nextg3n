...
class PPOStrategy(bt.Strategy):
    params = (('size', 100), ('stop_loss', 0.03), ('lookback', 20), ('confidence_threshold', 0.7))
    ...
    def next(self):
        ...
        if len(self.dataclose) < self.params.lookback:
            return
        forecast = self.dataclose[-1] / self.dataclose[-self.params.lookback] - 1
...
class BacktestEngine:
    ...
    async def run_backtest(self, symbol: str, timeframe: str = "1d") -> Dict[str, Any]:
        ...
        try:
            bars = await self.mcp_client.call_tool("polygon", "get_historical_bars", {"symbol": symbol, "timeframe": timeframe, "limit": 1000})
            if not bars["success"]:
                return {"success": False, "error": "No bars available", "symbol": symbol, "operation_id": operation_id}
            ...
            result = {
                "success": True,
                "symbol": symbol,
                "returns": cerebro.broker.getvalue() / 5000 - 1,
                "win_rate": 0.6,  # Placeholder
                "sharpe_ratio": 1.2,  # Placeholder
                "operation_id": operation_id,
                "timestamp": datetime.utcnow().isoformat()
            }
            self.redis.setex(f"backtest:{symbol}:{operation_id}", 86400, json.dumps(result))
            self.producer.send(
