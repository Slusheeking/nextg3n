...
class ContextRetriever:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.embedder_model = self.config.get("context", {}).get("embedder_model", 'all-MiniLM-L6-v2')
        self.summarizer_model = self.config.get("context", {}).get("summarizer_model", "ProsusAI/finbert")
        self.embedder = SentenceTransformer(self.embedder_model).to('cuda')
        self.summarizer = pipeline("summarization", model=self.summarizer_model)
        self.vector_db_collection_name = self.vector_db_config.get("collection_name", "nextg3n_vectors")
        self.vector_db = Client().get_or_create_collection(
            name=self.vector_db_collection_name,
    ...
    async def retrieve_context(self, symbol: str) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Retrieving context for {symbol} - Operation: {operation_id}")
        ...
        try:
            ...
            texts = []
            if news["success"]:
                texts.extend([a["title"] + " " + a["summary"]
                             for a in news["result"]["news"]])
            if reddit["success"]:
                texts.extend([p["title"] + " " + p["text"]
                             for p in reddit["result"]["posts"]])

            # Embed and store in vector DB
            embeddings = self.embedder.encode(
                texts, convert_to_tensor=True).to('cuda')
            ids = [f"{symbol}_{i}" for i in range(len(texts))]
            self.vector_db.add(ids=ids, embeddings=embeddings, metadatas=[
                               {"text": t} for t in texts])

            # Retrieve top contexts
            query_embedding = self.embedder.encode(symbol, convert_to_tensor=True).to('cuda')
            num_results = self.config.get("context", {}).get("num_results", 5)
            results = self.vector_db.query(query_embeddings=[query_embedding], n_results=num_results)
            ...
            redis_expiration_time = self.config.get("context", {}).get("redis_expiration_time", 300)
            self.redis.setex(f"context:{symbol}", redis_expiration_time, json.dumps(result))
            self.producer.send(
